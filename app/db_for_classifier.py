...
# Prompt the user for using Tkinter
def ask_to_use_tkinter():
    print("Tkinter is required for the graphical interface but may not work in a container environment.")
    response = input("Would you like to proceed with Tkinter? (yes/no): ").strip().lower()
    return response == 'yes'
...
# Main function for the Tkinter app (runs only if approved)
def main():
    if ask_to_use_tkinter():
        try:
            ...
            class DatabaseManagerUI:
                def __init__(self, root):
                    ...
                    self.init_button = ttk.Button(
                        ...
                        command=self.initialize_database_ui
                    )
                ...
                def initialize_database_ui(self):
                    initialize_database()
            ...
            print("Tkinter could not be imported. Ensure Tkinter is installed in your environment.")
    else:
        print("Tkinter usage was not approved.")
